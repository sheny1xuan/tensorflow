...
LiteRtStatus SumOpLegalization::LegalizeOp(LiteRtOpManager& src,
                                           Qnn_OpConfig_t& dest,
  ...
  auto src_axes_data = graph_tools::GetWeights<int32_t>(op_ins[1]).Value();
  LiteRtTensorManager::Unique src_axes;
  LITERT_RETURN_STATUS_IF_NOT_OK(
      LiteRtTensorManager::MakeFromTensor(op_ins[1], src_axes));
  // Check if src_axes are weights tensors.
  if (src_axes_data.empty()) {
    return kLiteRtStatusErrorInvalidLegalization;
  }
  int32_t dest_axes_size = src_axes->Dims()[0];
  LITERT_STACK_ARRAY(int32_t, axes_tensor_data, dest_axes_size,
                     /*init value=*/0);
  for (int i = 0; i < dest_axes_size; ++i) {
    axes_tensor_data[i] = src_axes_data[i];
  }
  Qnn_ClientBuffer_t axes_tensor_client_buf = BuildDefaultClientBuffer();
  axes_tensor_client_buf.data = axes_tensor_data;
  axes_tensor_client_buf.dataSize = dest_axes_size;

  // Extract keepdims option from sum op.
  bool keep_dims;
  LITERT_RETURN_STATUS_IF_NOT_OK(
      LiteRtSumGetKeepdimsOption(src.Op(), &keep_dims));
